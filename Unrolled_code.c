...
int main(int argc, char ** argv)
{
	...
	for(unsigned int j=0;j<iters;j++) // This is the one we will have to change
	{
		...
		for(unsigned int i=0;i<N;i+=UNROLL)
		{
			// Calculate num
			...
			num_0 = LVec[i + 1]+RVec[i + 1];
			num_1 = mVec[i + 1]*(mVec[i + 1]-1.0f)/2.0f;
			num_2 = nVec[i + 1]*(nVec[i + 1]-1.0f)/2.0f;
			num = num_0/(num_1+num_2);

			// Calculate den
			den_0 = CVec[i + 1]-LVec[i + 1]-RVec[i + 1];
			den_1 = mVec[i + 1]*nVec[i + 1];
			den = den_0/den_1;

			//Calculate w
			FVec[i + 1] = num/(den+0.01f);
			...
			num_0 = LVec[i + 2]+RVec[i + 2];
			num_1 = mVec[i + 2]*(mVec[i + 2]-1.0f)/2.0f;
			num_2 = nVec[i + 2]*(nVec[i + 2]-1.0f)/2.0f;
			num = num_0/(num_1+num_2);

			// Calculate den
			den_0 = CVec[i + 2]-LVec[i + 2]-RVec[i + 2];
			den_1 = mVec[i + 2]*nVec[i + 2];
			den = den_0/den_1;

			//Calculate w
			FVec[i + 2] = num/(den+0.01f);
			...
			num_0 = LVec[i + 3]+RVec[i + 3];
			num_1 = mVec[i + 3]*(mVec[i + 3]-1.0f)/2.0f;
			num_2 = nVec[i + 3]*(nVec[i + 3]-1.0f)/2.0f;
			num = num_0/(num_1+num_2);

			// Calculate den
			den_0 = CVec[i + 3]-LVec[i + 3]-RVec[i + 3];
			den_1 = mVec[i + 3]*nVec[i + 3];
			den = den_0/den_1;

			//Calculate w
			FVec[i + 3] = num/(den+0.01f);
